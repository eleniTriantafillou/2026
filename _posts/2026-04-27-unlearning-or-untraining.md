...
**Learning**, on the other hand, involves reducing expected risk on an underlying data distribution via training i.e., reducing the empirical risk on a finite dataset $\mathcal{D}$. In this sense, learning is synonymous with generalization -- the ability of making correct predictions on that underlying distribution of data, including unseen data, rather than just $\mathcal{D}$.
...
<div class="caption">
    ...
    In the middle of the figure, we depict the predictions of the original model that was trained on all of $\dataset$, before any <em>Un</em>&thinsp;learning or <em>Un</em>&thinsp;training is carried out. This model predicts correctly on all of its training set.
    On the left, we show the predictions of the model obtained by <em>Un</em>&thinsp;training $\forgetset$, which matches the behaviour of a model trained on $\dataset \setminus \forgetset$  -- i.e. a model trained on all of the data points except for the star and the two circles that belong to the forget set $\forgetset$.
    ...
    On the right, we show the predictions of the model obtained by <em>Un</em>&thinsp;learning the "behaviour" underlying $\forgetset$, where the "behaviour" in this case is "being a circle or a star". The unlearned model makes incorrect predictions on <em>all</em> examples of that behaviour.
</div>
